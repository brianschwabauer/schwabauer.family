...
export const actions: Actions = {
	default: async ({ request, platform }) => {
		...
		if (name.length > 80)
			return fail(400, { name, comment, error: 'Name is a bit long — keep it under 80 characters.' });
		if (comment && comment.length > 250)
			return fail(400, { name, comment, error: 'Comment must be 250 characters or less.' });
